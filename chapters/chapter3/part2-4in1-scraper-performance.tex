...
为考察弹簧压缩量对刮板刮削效果的影响，将各工况刮痕深度与弹簧压缩量的关系绘于图\ref{fig:scratch_depth_spring_comprassion}。综合图\ref{fig:scraper_test_rig}、图\ref{fig:scale_profile}、图\ref{fig:scraper_profile}与图\ref{fig:scratch_depth_spring_comprassion}所示的试验装置与刮削结果，可得如下结论。

（1）基于转盘的刮板刮削能力评价装置能够有效模拟刮板与套管内壁垢层间的相对刮削过程（图\ref{fig:scraper_test_rig}）。转盘旋转引起的相对运动在方向上与直线平动一致，沿刮刀跨度线速度的差异（相对平均速度最大约±19%）处于低速准静态范围，不改变硬垢的脆性崩碎去除机理；结合弹簧变形量测量与刮痕深度测量，可定量获得不同弹簧压缩量下的刮削深度，试验方法与评价指标可行。

（2）所设计的刮板能有效刮除以重晶石（硫酸钡）为主的硬垢。刮削后垢层表面可见连续、规则的刮痕，刀齿前刀面与容屑槽中堆积有粉末状和崩碎状切屑（图\ref{fig:scale_profile}），表明垢层以脆性崩碎方式去除，符合钡锶类硬垢的力学特性；且产屑量随弹簧压缩量的增大而增多，说明适当增大预紧力可提高单次刮削的去除量。

（3）刮痕深度随弹簧压缩量的变化呈“先较快增加、后趋于饱和”的非线性规律（图\ref{fig:scratch_depth_spring_comprassion}），二者之间并非简单正相关。压缩量较小时，刀齿以较小的实际接触面积压入垢面，局部接触应力大，刮痕深度随压缩量近似线性增大；当压缩量超过一定值后，沟槽经多次刮削逐渐贴合刮刀刃口轮廓，刀齿与垢面的实际接触面积增大，平均接触压应力的增幅趋缓，刮痕深度基本不再增大。与此同时，较大弹簧力下垢层表面发生压实变质并产生大量细屑，细屑充填前刀面与容屑槽，加剧刀齿堵塞（图\ref{fig:scraper_profile}），进一步限制了有效切削深度。
...
（4）因此，弹簧预紧力存在一个“有效区间”：落入该区间时刮削深度对预紧力较为敏感，超出该区间后继续增大预紧力对刮削深度的提升有限，反而会加剧刀齿堵塞与磨损。这一规律对工具设计有直接意义：刮刀外径与弹簧预紧量应针对套管的实际内径（同一公称尺寸不同线重时内径不同）进行匹配，使工具工作于该有效区间内，避免盲目增大预紧力；同时，现场作业宜配合循环冲洗及时排屑，以延缓刀齿堵塞，使刮板在有效区间内长时间保持切削能力。
\begin{figure}	
	\centering
	\includegraphics[width=.7\textwidth]{figure/chapter3/scatch-depth-sping-compression.pdf}
